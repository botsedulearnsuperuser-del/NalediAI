...
// Payment Processing screen - Figma node 2050:70
export default function PaymentProcessingScreen() {
  ...
  useEffect(() => {
    const processPayment = async () => {
      if (!params?.amount || !params?.qrData) {
      ...
      const { transaction, error } = await transactionService.send({
        amount,
        recipientPaymentCode: params.qrData,
      });
      ...
      setTimeout(() => {
        setProcessing(false);
        navigation.replace('PaymentComplete' as never, {
